...
def draw_molecule(
    coordinates, symbols, draw_bonds=None, save_location=None, dpi=300
):
    ...
    size = (
        np.array(plt.rcParams["lines.markersize"] ** 2)
        * 200
        / (len(coordinates))
    )
...
if __name__ == "__main__":
    ...
    for molecule in molecules:
        # == get molecule info ==#
        symbols, coords = open_xyz(
            os.path.join(("data/xyz"), molecule + ".xyz")
        )
        ...
        bond_histogram(
            ...
            save_location=os.path.join(
                os.getcwd(), f"data/{molecule}_bond_histogram.png"
            ),
        )
